...
| 2 | SCR-DASHBOARD | ダッシュボード | dashboard.html | 利用中サービス数・月額合計・クーポン残高などのサマリー、主要機能へのアクセスを容易にするクイックリンクセクション、およびお知らせを表示する。新たに「未読お知らせ」カードが追加され、直近7日以内のお知らせが未読としてカウントされる。 |
...
| 7 | SCR-CONTACT | お問い合わせ | contact.html | お問い合わせフォームを通じて、ユーザーが質問や問題を報告できる。サイドバーに「よくある質問」へのリンクが追加され、FAQページへのアクセスが容易になった。 |
